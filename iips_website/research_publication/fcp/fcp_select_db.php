...
<script type="text/javascript">
  $(document).on('click', '.edit_data', function(){  
                  var fcp_id = $(this).attr("id");
                  $.ajax({  
                            url:"research_publication/fcp/fcp_update_fetch.php",  
                            method:"POST",  
                            data:{fcp_id:fcp_id},
                            ...
                            {                               
                                 ...
                                 // $('#PPIJ_ID').val($getarray.PPIJ_ID);
                                  ...
                                  $('#FCP_TNO').val($getarray.Teach_FCP_TNO);  
                                  $('#FCP_BEP').val($getarray.Teach_FCP_DCP);  
                                  $('#FCP_ISSN').val($getarray.Teach_FCP_ISSN);  
                                  $('#FCP_NOC').val($getarray.Teach_FCP_NOC);  
                                  //$('#PPIJ_NCA').val($getarray.Teach_PPIJ_NCA);
                                  if($getarray.Teach_FCP_MA =="Yes")
                                    $("#FCP_Y").prop('checked', true);
                                  else
                                    $("#FCP_N").prop('checked', true);
                                  $('#fpcp_save').val("Update");
                            }  
   ...
   $(document).on('click', '.view_data', function(){  
                  var fcp_id = $(this).attr("id");
                  $.ajax({  
                            url:"research_publication/fcp/fcp_fetch.php",  
                            method:"POST",  
                            data:{fcp_id:fcp_id},
                            success: function(msg)
                            {                               
                                 var $getarray = jQuery.parseJSON(msg);
                                  ...
                                  $('#3').val($getarray.Teach_FCP_TNO);  
                                  $('#4').val($getarray.Teach_FCP_DCP);  
                                  $('#5').val($getarray.Teach_FCP_ISSN);  
                                  $('#6').val($getarray.Teach_FCP_NOC);  
                                  //$('#7').val($getarray.Teach_PPIJ_NCA);  
                                  $('#8').val($getarray.Teach_FCP_MA);  
  ...
  $(document).on('click', '.delete_data', function(){  
                  var fcp_id = $(this).attr("id");  
                  $.ajax({  
                            url:"research_publication/fcp/fcp_delete.php",  
                            method:"POST",  
                            data:{fcp_id:fcp_id},
                            success: function(msg)
                            {                               
  ...
  <center style="height: 400px; overflow: scroll;"><h3>Your Inserted Data</h3>
    <table class ="table table-striped" id="table_div"  style="border-color:#337ab7;" align="center" border="1px" >
     ...
     mysql_select_db('pbas_db');
      ...
      $query= "SELECT * FROM teach_fcp WHERE User_id = '$username'  AND (Data_Set='new' OR Data_Set='valid') ORDER BY fcp_id DESC";
      ...
      while($row=mysql_fetch_array($run)){
        ...
        $user_id= $row['User_id'];
        $fcp_id= $row['fcp_id'];
        $TNO= $row['Teach_FCP_TNO'];
        $DCP= $row['Teach_FCP_DCP'];
        $ISSN= $row['Teach_FCP_ISSN'];
        $NOC= $row['Teach_FCP_NOC'];
    ...
    <tr align="center">
        ...
        <td><button type="submit" name="view" id="<?php echo $row["fcp_id"]; ?>"  class="btn btn-success btn-xs view_data button_style" style="">View</button></a></td>

        <td><button type="submit" id="<?php echo $row["fcp_id"]; ?>" name="infodelete" class="btn btn-warning btn-xs delete_data button_style ">Delete</button></td>

        <td><a href="#slide_up"><button name="edit" value="Edit" id="<?php echo $row["fcp_id"]; ?>" class="btn btn-info btn-xs edit_data button_style">Update</button></a></td>
    </tr>
    <?php 
    }
    ?>
